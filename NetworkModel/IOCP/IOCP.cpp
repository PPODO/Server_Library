...
CONNECTION* IOCP::OnIOReceive(OVERLAPPED_EX* const pReceiveOverlapped, const uint16_t iRecvBytes) {
    if (auto pConnection = GetConnectionFromList(pReceiveOverlapped->m_pOwner)) {
        ...
        ReceiveDataProcessing(EPROTOCOLTYPE::EPT_TCP, pReceiveOverlapped->m_pReceiveBuffer, pReceiveOverlapped->m_iRemainReceiveBytes, pReceiveOverlapped->m_iLastReceivedPacketNumber, pConnection->m_pUser);
...
CONNECTION* IOCP::OnIOReceiveFrom(OVERLAPPED_EX* const pReceiveFromOverlapped, const uint16_t iRecvBytes) {
    if (auto pConnection = GetConnectionFromList(pReceiveFromOverlapped->m_pOwner)) {
        ...
        if (NETWORK::PROTOCOL::UTIL::UDP::CheckAck(*pReceiveFromOverlapped)) {
            ReceiveDataProcessing(EPROTOCOLTYPE::EPT_UDP, pReceiveFromOverlapped->m_pReceiveBuffer, pReceiveFromOverlapped->m_iRemainReceiveBytes, pReceiveFromOverlapped->m_iLastReceivedPacketNumber, pConnection->m_pUser);
